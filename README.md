...
Searching for multiple roads near a location proceeds as follows. First we identify the relevant latitude lists, which never exceeds 3 given the user's specified matching distance. Next we binary search the longitudinal endpoints on each list to cut out the appropriate range. In reality we are searching a square, with the boundaries of the square being determined in no more than 6 binary searches.
